...
The next process to consider is extinction.  Consider an island with $S=100$ species and an extinction rate of $\delta$. Update the code below to model the extinction of those species
...
#this line runs the model
extinct_dyn <- ode(init, times, extinction_mod, pars) 

plot(extinct_dyn[,1], extinct_dyn[,2], type='l', lwd=2, xlab="Time", ylab="Species")
```
...
#this line runs the model
full_dyn <- ode(init, times, islandBio_mod, pars) 

plot(full_dyn[,1], full_dyn[,2], type='l', lwd=2, xlab="Time", ylab="Species")
```
...
islandBio_mod <- function(t, state, pars) {
  with(as.list(c(state, pars)), {
    d_S <- r*(M-S) - delta0*exp(-delta*A)*S 
    return(list(c(S = d_S)))
  })
}
#this line runs the model
